...
func main() {
	...
	go func() {
		for {
			podList, err := clientset.CoreV1().Pods("default").List(context.TODO(), metav1.ListOptions{ResourceVersion: "0"})
			if err != nil {
				panic("Error get pods from cluster")
			}

			fmt.Printf("Total Pod: %v\n", len(podList.Items))
