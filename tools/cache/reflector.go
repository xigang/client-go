...
func (r *Reflector) Run(stopCh <-chan struct{}) {
	...
	go func() {
		for {
			klog.Infof("Last resource version: %v", r.relistResourceVersion())
...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	//1. 获取联邦集群所有的member集群列表
	//2. 并发获取所有member集群的资源数据，并replace indexer存储
	//3. 设置Reflector最新的resourceersion
	//4. 所有member集群List完成后，使用最新的resourceVersion对资源Watch

	clusters := []string{"cluster1", "cluster2"}
	clusterIdxCh := make(chan int, len(clusters))
	for i := 0; i < len(clusters); i++ {
		clusterIdxCh <- i
	}

	// `s` format: `{"cluster1":"123","cluster2":"456"}`
	requestResourceVersion := NewMultiClusterResourceVersionFromString(r.relistResourceVersion())
	...
	for i := 0; i < len(clusters); i++ {
		...
		go func() {
			defer wg.Done()
			select {
			case index := <-clusterIdxCh:
				labelSelector := metav1.LabelSelector{MatchLabels: map[string]string{"clustername": clusters[index]}}

				options := metav1.ListOptions{
					ResourceVersion: requestResourceVersion.Get(clusters[index]),
				...
				if err != nil {
					errCh <- fmt.Errorf("failed to list %v: %v", r.expectedTypeName, err)
				}
				...
				if err != nil {
					errCh <- fmt.Errorf("unable to understand list result %#v: %v", list, err)
				}
				...
				if err := r.syncWith(items, resourceVersion); err != nil {
					errCh <- fmt.Errorf("unable to sync list result: %v", err)
				}

				responseResourceVersionLock.Lock()
				defer responseResourceVersionLock.Unlock()
				mcr := NewMultiClusterResourceVersionFromString(resourceVersion)
				responseResourceVersion.Set(clusters[index], mcr.Rvs[clusters[index]])
			default:
				errCh <- nil
				return
			}
	...
	r.setLastSyncResourceVersion(responseResourceVersion.String())
	...
	for {
		...
		options := metav1.ListOptions{
			ResourceVersion: responseResourceVersion.String(),
		...
		watchRev := responseResourceVersion.String()

		if err := r.watchHandler(start, w, &watchRev, resyncerrc, stopCh); err != nil {
